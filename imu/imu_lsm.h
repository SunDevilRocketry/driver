/**
  ...
  *          HAL_SPI_TransmitReceive_DMA() handles non-blocking transactions.
  *          Post-transfer, imu_process_async_cb() must invalidate the D-Cache 
  *          region covering the rx buffer prior to parsing (SCB_InvalidateDCache_by_Addr).
  *          
  *          On transaction error, HAL_SPI_ErrorCallback() must route to
  *          imu_process_async_error_cb() to force CS high and release the shared SPI bus.
  * 
 ...
 *          - Matched Rates (e.g., 480 Hz ODR / 480 Hz SFLP):
 ...
 *            than SFLP slots. The DMA burst empties the reservoir, and parse_fifo_slot()
 *            decodes each slot via its hardware tag. No software-side synchronization is needed.
 */
 ...
 * @note  Sized to 16 slots to accommodate the worst-case backlog ratio of raw ODR
 *        (1.92 kHz) to SFLP rate (480 Hz):
 *        1.92 kHz ODR -> ~4 Accel + 4 Gyro + 3 SFLP slots = 11 slots per SFLP period,
 *        leaving 5 slots of conservative headroom.
 */
 ...
 *        Sensitivity values from DS14623 Rev3 Table 3.
 *
 * @warning Values are unique, sequential driver-internal integers and must NOT 
 *          be aliased to ST PID macros. The vendor's low-G and high-G full-scale 
 *          enums both start at index 0. Aliasing would cause value overlap collisions 
 *          (e.g., IMU_ACC_FS_32G overlapping with IMU_ACC_FS_2G), corrupting high-G 
 *          boundary checks and scaling calculations.
 *          
 *          Mapping to hardware register codes is handled internally via switch 
 *          translation statements in imu_init() and imu_set_accel_fs().
 */
